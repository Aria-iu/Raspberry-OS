...
/// PeripheralIC 结构体表示外设中断控制器，包括写寄存器 (wo_registers)、
/// 读寄存器 (ro_registers) 和中断处理程序表 (handler_table)。
...
impl PeripheralIC {
    /// new：构造函数，用于创建 PeripheralIC 实例。需要提供正确的 MMIO 起始地址。
...
///
/// 实现了 IRQManager 接口，用于中断管理。主要方法包括：
