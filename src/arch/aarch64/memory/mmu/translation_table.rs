...
/// PageDescriptor 描述的是指向实际物理内存的地址。
...
/// NUM_LVL2_TABLES 计算内核地址空间需要的二级翻译表的数量。
/// 它通过内核地址空间的大小除以 512MiB 的粒度大小来计算。
...
///
/// FixedSizeTranslationTable 是一个大结构体，用于存储翻译表。
///
/// 该结构体包含两个成员：
/// lvl3：一个二维数组，包含三级页描述符，每个描述符覆盖 64 KiB 的窗口。
/// lvl2：一个数组，包含二级表描述符，每个描述符覆盖 512 MiB 的窗口。
///
#[repr(C)]
#[repr(align(65536))]
...
///
/// 提供了两个方法用于创建和初始化 TableDescriptor 实例。
/// 这些方法帮助设置和管理内存翻译表项。
///
/// new_zeroed 是一个常量函数 (const fn)，用于创建一个新的 TableDescriptor 实例。创建的实例 value 字段被初始化为 0，这意味着该描述符是无效的（invalid）。
///
/// from_next_lvl_table_addr 用于创建一个指向指定地址的 TableDescriptor 实例
///
///
